...
def main():
    ...
    if not politicians:
        ...
        politicians = ["Donald J. Trump", "Joe Biden", "Kamala Harris", "John Roberts", "Ken Paxton", "Chuck Schumer", "Mitch McConnell", "Ron DeSantis", "Gavin Newsom", "Nancy Pelosi", "Kevin McCarthy", "Merrick Garland", "Alejandro Mayorkas", "Samuel Alito", "Clarence Thomas", "Elena Kagan", "Sonia Sotomayor", "Amy Coney Barrett", "Ketanji Brown Jackson"]  # expanded elected + high-impact for max profiles

    print(f"Building profiles for {len(politicians)} politicians...")

    # Load all processed data for aggregation (maximize: include politicians arena + all summaries/synthesis for cross-mentions)
    all_processed_keys = [k for k in list_processed() if "summary" in k or "synthesis" in k or "politicians" in k]
    all_data = []
