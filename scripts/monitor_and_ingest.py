...
def discover_education_new(s3, existing_keys: set) -> Generator[Tuple[str, str, str], None, None]:
    """Education (IPEDS, CCD)."""
    candidates = [
        ("education", "https://nces.gov/ipeds/use-the-data/download-access-database", "raw/education/ipeds-2024-25.zip"),
        ("education", "https://nces.gov/ccd", "raw/education/ccd-full.zip"),
    ]
    for arena, url, key in candidates:
        if key not in existing_keys and not key_exists(BUCKET, key, s3):
            yield arena, url, key
...
def discover_nhgis_metro_new(s3, existing_keys: set) -> Generator[Tuple[str, str, str], None, None]:
    ...
    # For auto, note as source; yield a sample or main page. For bulk, user can use their Data Finder for specific metro time series + boundaries + GIS for consistent metros over decades.
...
def discover_rss_news_new(s3, existing_keys: set, hours_back: int = 48, backfill: bool = False) -> Generator[Tuple[str, str, str], None, None]:
    ...
    - Expanded feeds (now ~20) for broader coverage incl. SCOTUS via courts, FEC filings, more congress/state gov signals, legacy media variants. All strictly output to R2 raw/news/.
    """
    # Curated ~19-20 high-value RSS feeds for the 11 arenas (lawfare/SCOTUS, congress, migration, bureaucracy, elections, media-tech, finance, education/culture, pharma/health etc.).
    # Prioritize .gov primary narrative sources (WH, DOJ, State, govinfo, congress.gov, fec.gov, uscourts) + major wires (reuters, ap, politico) + variants for media coordination detection (repeated framing/phrases across outlets) + state/legacy/think-adjacent signals.
    # Verified working examples (as of research): whitehouse presidential-actions feed, justice.gov news/pr rss, politico rss, reuters feeds, state.gov, govinfo fr, ap feeds, congress.gov. Expanded: fec.gov efiling, us courts news (SCOTUS/fed judiciary), additional congress.gov rss (most-viewed, presented), reuters world/business variants, ap-us-news, govinfo-bills.
    # Keep minimal: fetch via requests + parse with xml.etree.ElementTree (stdlib); no feedparser dep added.
    # All output tied to R2 babylon-raw-data/raw/news/ (article HTMLs) + raw/media/rss-*.xml (feed snapshots) for liveness + downstream process/analyze/news_ripples.
    feeds = [
        ...
        # NEW 7 high-value for expanded political/news coverage (SCOTUS, FEC filings, state gov signals via more congress/fed, more legacy AP/Reuters variants)
        ("fec-filings", "https://efilingapps.fec.gov/rss/"),  # FEC electronic filings RSS (elections/campaign finance arena, direct fec.gov)
        ("us-courts-news", "https://www.uscourts.gov/news/rss"),  # US Courts newsroom feed (SCOTUS + federal judiciary signals, lawfare arena)
        ("congress-most-viewed", "https://www.congress.gov/rss/most-viewed-bills.xml"),  # congress.gov most-viewed (public interest/elections arena)
        ("congress-presented", "https://www.congress.gov/rss/presented-to-the-president.xml"),  # bills presented to president (congress/exec)
        ("reuters-world", "http://feeds.reuters.com/Reuters/worldNews"),  # Reuters world/international (foreign/migration arenas; more legacy wire variant)
        ("reuters-business", "http://feeds.reuters.com/Reuters/businessNews"),  # Reuters business/finance (econ/finance arena; AP/Reuters variants)
        ("ap-us-news", "https://feeds.apnews.com/rss/ap-us-news"),  # Additional AP legacy media US news variant for broader coverage
        ("govinfo-bills", "https://www.govinfo.gov/rss/bills.xml"),  # Govinfo congressional bills (congress/bureaucracy primary; expands govinfo-fr)
    ]
    ...
    for feed_name, feed_url in feeds:
        try:
            ...
            for item in items:
                ...
                # TIED TO R2: all keys written to babylon-raw-data/raw/news/ (HTMLs for ingest/process/analyze) or raw/media/rss- (XML snapshots). raw/news/ prefix scanned in list_existing + health checks + R2 verifier.
    ...
    # Liveness logging update (Discover Sources Maximizer subagent): expanded coverage injected
    print(f"  [NEW-DISCOVER] RSS liveness now on {len(feeds)} feeds (was ~12; +FEC, SCOTUS/us-courts, congress variants, reuters/ap more, govinfo-bills). Covers 11 arenas + filings/state/legacy. All to R2 raw/news/. Keep on.")
...
SOURCE_MAP = {
    ...
    # SOURCE_MAP entry for rss_news (updated by Discover Sources Maximizer: now covers expanded feeds for 11 arenas + SCOTUS/FEC/etc; liveness logs injected; tied to R2 raw/news/)
...
def run_monitor(sources: List[str] = None, max_new: int = 10, dry_run: bool = False, arena_filter: str = None,
    ...
    # R2 raw/news/ tie-in: dedicated prefix for rss_news discovers (articles + feed XMLs archived here for liveness + synthesis)
    existing = set()
    ...
    if "rss_news" in (sources or []) or rss_sources_used:
        ...
        # Updated liveness summary (post SOURCE_MAP/feeds expansion): reports added coverage for SCOTUS/FEC/etc into R2 raw/news/
        print(f"SUBAGENT #NEW-DISCOVER liveness: expanded feeds active; see per-feed + total in discover_rss_news_new logs. Tied explicitly to babylon-raw-data/raw/news/ + raw/media/rss-*.")
...
def main():
    ...
    parser.add_argument("--sources", default="gdelt,commoncrawl,hf-reddit,ia-twitter,lobbying,patents,health,education,legal,global,general,census_metro,bls_qcew,nyc_open,ca_open,nhgis_metro,eurostat_nuts,court_documents,press_releases,crs_gao_reports,foia_documents,politicians_fec,politicians_congress,politicians_courts_recap,politicians_state_elections,politicians_opensecrets,rss_news",
                        help="Comma-separated sources to check (broad default for 'just do it' - now includes state/local/metro + dedicated documents sources for court rulings, press releases, CRS/GAO reports, FOIA etc. filling raw/documents/ + rss_news for liveness under raw/news/ + raw/media/rss-... ; rss_news expanded +8 feeds by Discover Sources Maximizer subagent for SCOTUS/FEC/congress variants/legacy wires)")
