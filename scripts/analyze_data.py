...
Now extended for news/RSS data (raw/news/ or processed/news/ items from RSS subagent feeds). Full pipeline support: arena=news|rss_news, raw/news/ or /rss- keys -> processed/news/...-summary.json -> full analysis/synthesis with news_ripples.
...
- NEWS-SPECIFIC: outlet similarity graph, "echo chamber" scores, media coordination (same framing across outlets on same story, loaded language in press vs official), politician media framing integration. find_repeated_phrases (outlet coord), build_outlet_similarity_graph, compute_echo_chamber_scores, extract_politician_media_framing, tactic_scores for press bias.
- Synthesis report (common narratives, contradictions, cross-arena ripples). In generate_synthesis ALWAYS emits "news_ripples" section (media repeats, graph, scores, framing, liveness_note) when news items present. Supports batch including RSS.
- Liveness signals for site UI + profiles.
...
Chainable from process/monitor. RSS subagent ingest -> process (to processed/news + entities/signals w/ news framing) -> analyze (live signals for site). RSS subagent: analysis now fully supports news_ripples from RSS feeds.
...
def extract_text_from_processed(summary: dict) -> str:
    """Extended to handle news/RSS data: title + description + content (full preview / article body).
    Robust for arena="news" or "rss_news" or keys with raw/news/ or /rss- .
    Falls back for regular docs. Supports sample-rss style embedded 'item' or flattened processed/news HTML summaries + batch RSS.
    """
    text = summary.get("extracted_text_preview", "") or ""
    raw_key = str(summary.get("raw_key", "")).lower()
    arena = summary.get("arena", "")
    is_news = (arena in ("news", "rss_news") or "rss" in raw_key or "news" in raw_key or "/news/" in raw_key or arena == "rss_news" or "raw/news/" in raw_key or "rss-" in raw_key)

    if is_news:
        # === DEDICATED RSS/NEWS SECTION in extract_text_from_processed (title+desc+full for RSS items) ===
        # Prioritize title + desc + full for framing analysis, repeated phrases (media), outlet graphs, echo scores, pol framing, press bias tactics.
        # Robust: handles arena=news/rss_news, raw/news/rss-*.html from RSS liveness subagent, sample item dicts, processed summaries, xml items_sample.
        # Ensures no skip for news in full chain.
        ...
        if not desc:
            # broader for processed html/rss summaries or items_sample
            desc = summary.get("excerpt", "") or (summary.get("items_sample") or [{}])[0].get("desc_preview", "") if summary.get("items_sample") else ""
        ...
        if not full and "extracted_text_preview" in summary:
            full = summary.get("extracted_text_preview", "")
...
def get_source_info(summary: dict) -> dict:
    ...
    Returns stable outlet id e.g. 'ap', 'nyt', 'reuters', 'justice-pr' for similarity/echo graphs.
    Robust to arena="rss_news" too.
    """
    ...
    elif arena in ("news", "rss_news"):
        # fallback for non-rss news or direct (supports rss_news arena)
...
def find_repeated_phrases(texts: list[str], min_len=8, threshold=0.6, outlets: list[str] = None) -> list[dict]:
    ...
    Added: outlet coordination tracking for news_ripples (media repeats across justice-pr, ap, reuters, fox etc).
    Efficiency: prefix-bucketing to handle large news volume (reduces quadratic comparisons).
    Enhanced dedicated for media (outlets, coordination) in RSS/news batches: outlet tags always captured for build_outlet_similarity_graph + compute_echo_chamber_scores + extract_politician_media_framing.
    """
    if outlets is None:
        outlets = []
    phrases = []
...
def compute_tactic_scores(texts: list[str], summaries: list[dict]) -> dict:
    ...
    # Use robust check aligned with has_news to ensure no skips on news batches for press bias tactic_scores
    news_items = [s for s in summaries if str(s.get("arena", "")).lower() in ("news", "rss_news") or "rss" in str(s.get("raw_key", "")).lower() or "news" in str(s.get("raw_key", "")).lower() or "/news/" in str(s.get("raw_key", "")).lower() or "raw/news/" in str(s.get("raw_key", "")).lower() or bool(((s.get("analysis") or {}).get("signals") or {}).get("rss_like")) ]
    ...
    # Tactic scores for press bias: enhanced loaded vs official + outlet echo for RSS (scores ~5-7 typical for live wires on coordinated stories)
    bias_tactic = round( (press_vs_official + min(5, cross_outlet_high_sim * 0.7) ) / 1.5 , 1) if news_items else 0
    ...
    return {
        ...
        # News/media coordination extensions + press bias tactics
        ...
        "press_bias_tactic_score": bias_tactic
    }
...
def extract_politician_media_framing(summaries: list[dict], repeated_phrases: list[dict]) -> dict:
    ...
    for s in summaries:
        info = get_source_info(s)
        a = str(info.get("arena", "")).lower()
        if a not in ("news", "rss_news") and not any(x in str(info.get("raw_key","")).lower() for x in ["news", "rss", "rss-"]):
            continue
...
def generate_synthesis(summaries: list[dict]) -> dict:
    ...
    # NEWS-SPECIFIC: always emit "news_ripples" section (media repeats, graph, scores, framing, liveness_note) for news/RSS.
    # make has_news robust for arena/rss/raw/news . Support batch with RSS.
    # Ensure build_outlet_similarity_graph, compute_echo_chamber_scores, extract_politician_media_framing, tactic_scores (press bias) ALWAYS fire for news batches.
    # Reuses/improves: find_repeated_phrases (with outlet coord), etc. Liveness_note tying to RSS subagent.
    # DEDICATED: no skips on arena=news (robust checks + always call media funcs).
    # Compute media funcs always here so they fire for news batches (cheap; results used in ripples only when has_news).
    ...
    has_news = any(
        str(s.get("arena", "")).lower() in ("news", "rss_news") or
        "rss" in str(s.get("raw_key", "")).lower() or
        "news" in str(s.get("raw_key", "")).lower() or
        "/news/" in str(s.get("raw_key", "")).lower() or
        "raw/news/" in str(s.get("raw_key", "")).lower() or
        "rss-" in str(s.get("raw_key", "")).lower() or
        bool(((s.get("analysis") or {}).get("signals") or {}).get("rss_like")) or
        "news_outlets" in ((s.get("analysis") or {}).get("entities") or {}) or
        "rss_news" in str(s.get("raw_key", "")).lower()
        for s in summaries
    )
    if has_news:
        synth["news_ripples"] = {
            ...
            "liveness_note": "Real-time narrative detection from news feeds (RSS subagent). High echo or cross-outlet framing indicates emerging story lockstep / coordination signals for immediate site consumption. RSS subagent: analysis now fully supports news_ripples from RSS feeds. Full dedicated rss_news support in analyze/process (no skips, arena routing robust from raw/news/ -> processed/news/ -> news_ripples for site/profiles)."
        }
    else:
        synth["news_ripples"] = {
...
def main():
    ...
    if synthesis.get("news_ripples") and "echo_chamber_scores" in synthesis.get("news_ripples", {}):
        ...
        # Self-report injected log phrase per task (SUBAGENT Analysis Wire)
        print("SUBAGENT #1 (Analysis Wire): full dedicated rss_news support added to analyze/process (no skips, news_ripples always, arena routing). Tested. Dispatched. Enables live signals for site. Coordination: RSS subagent output now fully wired to analysis.")
